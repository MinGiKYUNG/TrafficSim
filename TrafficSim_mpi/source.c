	...
	for (int link = 0; link < numLink; link++) {
		for (int sect = 1; sect < NUM_SECTION + 1; sect++) {
			for (int lane = 0; lane < NUM_LANE; lane++) {
				...
				fprintf(fp, "%d_", sect);
				fprintf(fp, "%d_", lane);
				fprintf(fp, "%f_", l[link].lenSection[sect] * NUM_SECTION);
				fprintf(fp, "%d_", l[link].numVehArr[sect][lane]);
				fprintf(fp, "%f_", l[link].ffSpeed);
				fprintf(fp, "%f_", l[link].lenSection[sect]);
				fprintf(fp, "%f_", l[link].density[sect][lane]);
...
int main(int argc, char *argv[])
{
   ...
   /* Initialize the MPI library */
    ...
    if (myrank == 0) {
	printf("Simulation Node 0 initialized.\n");
...
numLink = 1000;
	...
	int* linkIDarr = malloc(numLink * sizeof(int));
	memset(linkIDarr, 0, numLink * sizeof(int));
	int* vehTotIDarr = malloc(numVeh * sizeof(int));
	memset(vehTotIDarr, 0, numVeh * sizeof(int));
	int* ccIDarr = malloc(numLink * sizeof(int));
